...
개발자란 자투리 시간이 생길때마다 해야하는 중요한 행동들이 있다.
...
==즉, 우리 컴퓨터는 네이버 같은 웹페이지를 호스팅해주는 서버로부터 HTML, CSS, Javascript 파일을 다운로드하는 것이다.==
...
google material design이 나오고 나서 웹사이트 디자인이 모던하고 트랜디하게 바뀌었는데 MKDocs-materials이 이 디자인 테마를 제공해준다. 따라서 우리는 이 도구를 사용하여 모던하고 예쁜 블로그 사이트를 만들 수 있다.
...
이번 포스팅에서는 설치 방법이나 배포 방법에 대해 깊게 다루지 않고 이 개발 블로그를 MKDocs-materials를 사용하여 배포할 때 있었던 시행착오와 이 블로그 template이 얼마나 효율적인 시간 절감을 이뤄줬는 지 소개하려고 한다.
